...
class Course
{
   ...
   function getSomeWithFilters($offset, $num, $filters)
   {
      try {
         $sql = 'SELECT c.course_id, c.title, c.description, c.category, c.credits, 
                        c.created_at, c.cover, c.instructor_id,
         ...
         if (isset($filters['credits']) && $filters['credits'] !== '') {
            $sql .= ' AND c.credits = ?';
            $params[] = (int) $filters['credits'];
         }
         ...
         // Debug logging
         error_log("Filter SQL: " . $sql);
         error_log("Filter Params: " . print_r($params, true));

         $stmt = $this->conn->prepare($sql);
         $stmt->execute($params);

         $courses = $stmt->fetchAll(PDO::FETCH_ASSOC);
         error_log("Found courses: " . count($courses));

         return !empty($courses) ? $courses : 0;
      } catch (PDOException $e) {
   ...
   {
      try {
         $sql = 'SELECT COUNT(*) as total FROM ' . $this->table_name . ' c
                 LEFT JOIN instructor i ON c.instructor_id = i.instructor_id 
                 WHERE 1=1';
         $params = [];
   ...
   function createStudentProgress($cour_id, $stud_id, $val)
   {
      try {
         ...
         $existing = $this->getStudentProgress($cour_id, $stud_id);
         if ($existing) {
            // If exists, update instead of create
            return $this->updateStudentProgress($cour_id, $stud_id, $val);
         }
   ...
   function deleteByInstructorId($instructor_id)
   {
      try {
         $sql = 'DELETE FROM course WHERE instructor_id=?';
         $stmt = $this->conn->prepare($sql);
         return $stmt->execute([$instructor_id]);
      } catch (PDOException $e) {
         return 0;
      }
   }
   ...
   function getEnrolledStudents($course_id)
   {
      try {
         $sql = 'SELECT s.* FROM student s 
                 INNER JOIN enrolled_student e ON s.student_id = e.student_id 
                 WHERE e.course_id = ?';
         $stmt = $this->conn->prepare($sql);
         $stmt->execute([$course_id]);
   ...
   function getEnrolledCount($course_id)
   {
      try {
         $sql = 'SELECT COUNT(*) as count FROM enrolled_student WHERE course_id = ?';
         $stmt = $this->conn->prepare($sql);
         $stmt->execute([$course_id]);
         $result = $stmt->fetch();
         return $result['count'];
   ...
   public function getEnrolledCoursesWithGrades($student_id)
   {
      try {
         $sql = "SELECT c.course_id, c.title, c.description, c.cover, c.created_at, 
                        IFNULL(sp.progress, 0) as progress,
                        i.first_name as instructor_first_name, 
                        i.last_name as instructor_last_name
                 FROM enrolled_student es
                 INNER JOIN course c ON es.course_id = c.course_id
                 LEFT JOIN student_progress sp ON (sp.course_id = c.course_id AND sp.student_id = es.student_id)
                 LEFT JOIN instructor i ON c.instructor_id = i.instructor_id
                 WHERE es.student_id = ?";
         $stmt = $this->conn->prepare($sql);
