...
class Student
{
    ...
    function delete($student_id)
    {
        try {
            ...
            // Delete associated records from enrollment table first
            $sql1 = 'DELETE FROM enrolled_student WHERE student_id = ?';
            $stmt1 = $this->conn->prepare($sql1);
            $stmt1->execute([$student_id]);

            // Delete associated records from certificate table
            $sql2 = 'DELETE FROM certificate WHERE student_id = ?';
            $stmt2 = $this->conn->prepare($sql2);
            $stmt2->execute([$student_id]);

            // Finally delete the student
            $sql3 = 'DELETE FROM ' . $this->table_name . ' WHERE student_id = ?';
            $stmt3 = $this->conn->prepare($sql3);
            $res = $stmt3->execute([$student_id]);
